...
本项目支持多个 LLM 提供商（Anthropic、OpenAI），可在 `config.yaml` 中配置模型和提供商。但当前仅使用单一模型，调用失败时会直接报错。

- **建立模型池**：配置多个模型账号和提供商，建立模型池以提高服务可用性。
- **引入高可用策略**：为模型池引入自动健康检测、故障节点切换、熔断等高可用策略。
- **跨提供商自动回退**：当主模型不可用时，自动切换到其他提供商（如 Anthropic → OpenAI）。
